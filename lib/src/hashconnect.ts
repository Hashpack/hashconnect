...
export class HashConnect implements IHashConnect {
  ...
  async disconnect(topic: string) {
    ...
    let index = this.hcData.pairingData.findIndex(
  ...
  createOperatorSigner(accountId: string) {
    const operatorSigner = async (txBodyBytes: Uint8Array) => {
      ...
      if (typeof res.signedTransaction === "string") {
        throw new Error(
          `Hashconnect unexpectedly returned a signedTransaction of type string`
        );
      }

      if (res.signedTransaction === undefined) {
        throw new Error(
          `Hashconnect unexpectedly returned an undefined signedTransaction`
        );
      }
